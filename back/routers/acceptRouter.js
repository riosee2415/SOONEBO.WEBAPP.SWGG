...
router.post("/list/date", isAdminCheck, async (req, res, next) => {
  const { searchDate, dName } = req.body;
...
router.post("/list/graph", isAdminCheck, async (req, res, next) => {
  const { typeId, dName } = req.body;
    ...
    SELECT 	A.createdAt		AS date,
      ...
      FROM	(
              SELECT	DATE_FORMAT(createdAt, '%Y-%m-%d') as createdAt 
               ...
               WHERE  ${
                 _dName !== `main` ? `dName = "${_dName}"` : `dName = "main"`
               }
                ${condition}
            ) A
